  ...
  return (
    <div className="rounded-xl bg-[#12121e] border border-[#1e1e30] p-5">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-[11px] font-semibold uppercase tracking-widest text-slate-500">Goals</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {goals.map(goal => (
            <a
              ...
              className="p-3.5 rounded-lg bg-[#0f0f17] border border-[#1e1e30] hover:border-teal-500/30 group block"
            >
              {/* Name + badge */}
              <div className="flex items-start justify-between gap-2 mb-2">
                <span className="text-sm font-medium text-slate-200 group-hover:text-teal-400 leading-snug flex-1">
                  {goal.name}
                </span>
                <span
                  className={`shrink-0 text-[10px] px-2 py-0.5 rounded-full border font-medium ${statusChip(goal.status)}`}
                >
                  {goal.status}
                </span>
              </div>
